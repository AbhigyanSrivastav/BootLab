...
export function PlatformSelection({
  ...
  const [selectedPlatform, setSelectedPlatform] = useState<ImageOption | null>(
    null
  );
  ...
  const handleLaunchPlatform = async () => {
    ...
    try {
      ...
      setIsModalOpen(false);
      setErrorMessage(
        "Failed to launch platform. Please try again later." + err
      );
    ...
    <div className="min-h-screen">
      <div className="bg-gray-900/95 backdrop-blur-sm border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          >
            <div
              className={`absolute inset-0 bg-gradient-to-r from-white/20 to-transparent opacity-0 transition-opacity duration-300 rounded-2xl ${
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">
            Explore ready-to-launch environments tailored for development,
            testing, browsing, and productivity — all in the cloud.
          </p>
        </div>
        {/* fallback */}
        {loading && (
          ...
          <div className="space-y-12">
            {options.map((group) => (
              <PlatformSection
                key={group.type}
                title={group.label}
                description={`Run ${group.label.toLowerCase()} instantly in the cloud`}
